...
function AppContent() {
  ...
  return (
    <>
      ...
      >
        <Routes>
          <Route path="/" element={<Home isEditing={isEditing} onHasChanges={setHasChanges} registerSaveAll={(fn) => { saveAllRef.current = fn; return true; }} onSiteNameChange={setSiteName} onOpenSidebar={() => setSidebarOpen(true)} onCanEditChange={setCanEdit} onOnboardingComplete={() => { setIsEditing(true); setCanEdit(true); }} onCloseSidebar={() => setSidebarOpen(false)} />} />
