...
import { migrateProperty } from '../services/migratePropertyService';
...
export function OnboardingPopup({ onComplete, onImported, onClose, scrapedProperty, scrapedImages, onSiteNameChange, onOpenSidebar }: OnboardingPopupProps) {
 ...
 const handleSaveSiteInPopup = async () => {
   ...
   try {
     ...
     // ── Push scraped data to the migration property in Supabase ──
     if (scrapedData) {
       try {
         ...
         console.log('[PopupSaveSite] ✅ Migration property updated');
       } catch (migErr) {
         console.warn('[PopupSaveSite] ⚠️ Migration error (non-fatal):', migErr);
       }
     }
     ...
     // Trigger Railway build + deploy to Hostinger
...
<br />
 ...
 {/* Deploy triggered — Railway is building your site */}
 ...
 <p style={{ color: '#888', fontSize: '0.8rem', margin: '0 0 12px 0' }}>
 Deploy webhook not configured — site saved but not yet live.
 </p>
 )}
 {deployUrl && (
